...
import com.gary.payment.account.unionpay.UnionPayUPOPThirdAccount;
...
/**
 * 银联无卡支付
 * @ClassName: UnionPayUPOP 
 * @Description:  
 * @author zhouxianjun(Gary)
 * @date 2014-12-19 下午2:07:44 
 *
 */
public abstract class DefaultUnionPayUPOP extends AbstractRechargeService implements RechargeService {
	
	@Autowired(required = false)
	private UnionPayUPOPThirdAccount unionPayUPOPThirdAccount;
	...
	public OrderRequest payment(PayRequest request) throws PaymentException {
		...
		String merId = unionPayUPOPThirdAccount.getMerId();
		String curyId = unionPayUPOPThirdAccount.getCuryId();
		String transDate = Utils.toYYYYMMDD(new Date());
		String transType = unionPayUPOPThirdAccount.getTransType();
		...
		parmMap.put("CpVersion", unionPayUPOPThirdAccount.getVersion()); //接入版本号V4
		parmMap.put("CuryId", curyId); //订单交易币种，3位长度，固定为人民币156
		parmMap.put("GateId", ""); //支付网关号
		parmMap.put("PageRetUrl", unionPayUPOPThirdAccount.getPageRetUrl()); //页面交易接收URL，长度不要超过80个字节
		parmMap.put("BgRetUrl", unionPayUPOPThirdAccount.getBgRetUrl()); //后台交易接收URL，长度不要超过80个字节
		String priv1 = "";
		...
		orderRequest.setPayUrl(unionPayUPOPThirdAccount.getPayUrl());
		return orderRequest;
	}
	
	public PayResponse response(String type, Map<String, String> response) {
		logger.info("返回银联无卡支付信息:{}", response);
		//支付订单数据准备
		...
		if(verify && "1001".equals(Status)){
			logger.info("银联无卡支付充值结果:充值订单号{},充值返回值{},支付网关{}", new Object[] { OrdId, Status, GateId});
	...
	private PrivateKey getPrivateKey(String MerId) {  
		try {
			...
			String path = unionPayUPOPThirdAccount.getPrivateKey();
	...
	private PrivateKey getPrivateKey(){
		try {
			boolean buildOK = false;
			PrivateKey key = new PrivateKey();
			String path = unionPayUPOPThirdAccount.getPublicKey();
